...
if [ -f "$app_path/DLLs/MSSTDFMT.DLL" ]; then
    echo "✅ O arquivo existe: $app_path/DLLs/MSSTDFMT.DLL"
    ...
    echo "❌ O arquivo não existe: $app_path/DLLs/MSSTDFMT.DLL"
...
if [ "$acao" = "GIA RS" ] ; then
  ...
  sleep 1
  mkdir -p /opt/projetus/facilitador/cache/atualizacao
