...
for cmd in git notify-send tar xz unzip fc-list sed python3 curl ping wget sudo flatpak pkexec find firefox  gettext ; do

    if ! command -v "$cmd" > /dev/null 2>&1; then


        # message=$(gettext "Erro: %s não está instalado.")
        message="Erro: %s não está instalado."
        ...
        yad --center --window-icon="$logo" --title="$titulo" --text="$message" --buttons-layout=center --button=OK:0 --width="300" --height="100" 2> /dev/null
...
# Verifica se tem slackpkg (Slackware)
...
if command -v slapt-get >/dev/null 2>&1; then
        ...
        echo -e "\n📦 Salix detectado. \n"
            ...
            # sudo slapt-get --search openjdk

            sudo slapt-get --install -y OpenJDK17
...
check_font() {
  ...
  if fc-list | grep -iq "Noto Color Emoji"; then
...
# Função para instalar a fonte de emoji

install_font() {

  DISTRO=$(lsb_release -si)

  echo -e "\nFonte Noto Color Emoji não encontrada. Instalando... \n"

  case $DISTRO in

    "Arch" | "Manjaro" | "EndeavourOS"| "biglinux")
      # Pacote para Arch Linux e derivados
      sudo pacman -S --noconfirm noto-fonts-emoji
      ;;

    "Ubuntu" | "Debian" | "LinuxMint")
      # Pacote para Ubuntu/Debian e derivados
      sudo apt update
    ...
    "openSUSE")
      # Pacote para openSUSE
      sudo zypper install -y noto-fonts-emoji
      ;;

    "VoidLinux")
      # Pacote para VoidLinux
      sudo xbps-install -Suvy noto-fonts-emoji
      ;;

    *)

      echo -e "\nDistribuição não suportada diretamente por este script. \n"
      echo -e "Tente instalar a fonte manualmente. \n"
...
echo "Verificando se a fonte Noto Color Emoji está instalada..."

if check_font; then

  echo -e "\nA fonte Noto Color Emoji já está instalada! \n"
...
if command -v wine >/dev/null 2>&1; then
    ...
    case "$DISTRO" in
        ...
        arch|manjaro|biglinux)
            ...
            # Atualiza seu sistema (-Syu).

            sudo pacman -Syu --noconfirm wine


            # 🍷 Instalar Wine com suporte completo (32 e 64 bits)


            # winetricks → facilita instalar bibliotecas do Windows (.dlls, fontes etc.)

            # wine-mono / wine-gecko → fornecem substitutos de .NET e do Internet Explorer usados por alguns programas.
...
# 🍷 Testar Wine após a instalação

# winecfg

# Isso criará a pasta ~/.wine e abrirá o painel de configuração do Wine.
