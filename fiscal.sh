...
if [ "$acao" = "DMA BA" ]; then
  ...
  executar "env WINEARCH=win32 WINEPREFIX=$HOME/.wine32 winetricks --force  gdiplus tdbg32 msvbvm60 vb6run"
...
if [ "$acao" = "EFD Contribuições" ]; then
  ...
  executar "sh $cache_path/EFDContribuicoes.sh"
...
if [ "$acao" = "SPED ICMS IPI" ]; then
  ...
  # download "https://servicos.receita.fazenda.gov.br/publico/programas/Sped/SpedFiscal/PVA_EFD_linux-5.0.0_x64.jar" "$cache_path/PVA_EFD.jar"
  ...
  download "https://servicos.receita.fazenda.gov.br/publico/programas/Sped/SpedFiscal/SpedEFD_linux_x86_64-5.0.3.sh" "$cache_path/PVA_EFD.sh"
  ...
  cp "$user_path"/Fiscal/ProgramasSPED/EFD ICMS IPI.desktop "$desktop_path/Validadores"
...
if [ "$acao" = "SINTEGRA" ]; then
  # configurarWine
  download "https://objectstorage.sa-saopaulo-1.oraclecloud.com/n/id3qvymhlwic/b/downloads/o/facilitador-linux%2Fsintegra.exe" "$cache_path/sintegra.exe"
...
if [ "$acao" = "DIEF MA" ] ; then
  ...
  #setWinePrefix "$win" "$stricks"
  configurarWine 
  download "--header='Accept: text/html' --user-agent='Mozilla/5.0 (Macintosh; Intel Mac OS X 10.8; rv:21.0) Gecko/20100101 Firefox/21.0' http://downloads.sefaz.ma.gov.br/diefportal/Instalador_DIEF64_64bits.EXE"  "$cache_path/Instalador_DIEF64_32bits.EXE"
