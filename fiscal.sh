...
if [ "$acao" = "EFD Contribuições" ]; then
  ...
  user_install "EFD_Contribuições"
...
if [ "$acao" = "GIA SP" ]; then
  ...
  executar "env WINEARCH=win32 WINEPREFIX=$HOME/.wine32 winetricks dotnet20 dotnet40"
...
if [ "$acao" = "GIAM TO" ]; then
  ...
  rm -Rf GIAM*
  cd /opt/projetus/facilitador
  download "http://giam.sefaz.to.gov.br/download/Instalargiam10.0_28.02.2019v1.exe" "$cache_path/giamto.exe"
  ...
  user_install "GIAM_TO"

  endInstall
fi

if [ "$acao" = "SPED ICMS IPI" ]; then
  download download "http://www.receita.fazenda.gov.br/publico/programas/Sped/SpedFiscal/PVA_EFD_linux-2.6.7_x64.jar" "$cache_path/PVA_EFD.jar"
...
if [ "$acao" = "DAC AL" ]; then
  configurarWine
  download  "--header=""Accept: text/html"" --user-agent=""Mozilla/5.0 (Macintosh; Intel Mac OS X 10.8; rv:21.0) Gecko/20100101 Firefox/21.0"" http://gcs.sefaz.al.gov.br/documentos/visualizarDocumento.action?key=hPq2RHFPMiM%3D" "$cache_path/InstalaDAC221011.exe"
...
if [ "$acao" = "Livro Eletronico GDF" ] ; then
  ...
  rm -rf  ~/.local/share/applications/wine/Programs/Validador*
  user_install "Livro_Eletronico_GDF"
