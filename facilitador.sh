...
if [ "$setor" = "Contábil" ]; then ## Contabil
    ...
    TRUE "SPED ECD" "Versão 7.0.6" \
    FALSE "SPED ECF" "Versão 6.0.6" \
  ...
  acao=$(zenity  --list  --text "Selecione o programa desejado:" \
    ...
    FALSE "EFD Contribuições" "Versão 4.0.1" \
  ...
  acao=$(zenity  --list  --text "Selecione o programa desejado:" \
    ...
    --column="" --column "Programa"  --column "Descrição"\
    TRUE "Calima App - Acesso Local" "Versão 1.1.0"\
    FALSE "Calima App - Acesso Web" "Versão 1.1.0"\
    FALSE "Calima Server" "Versão 3.0.0"\
