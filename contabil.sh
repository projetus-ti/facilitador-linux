...
if [ "$acao" = "SPED ECD" ]; then
  download "http://www.receita.fazenda.gov.br/publico/programas/Sped/SpedContabil/SPEDContabil_linux_x64-7.0.6.jar" "$cache_path/SPEDContabil.jar"
  ...
  user_install "SPED_ECD%207.0.6"

  endInstall
fi

if [ "$acao" = "SPED ECF" ]; then
  download "http://www.receita.fazenda.gov.br/publico/programas/Sped/ECF/SpedEcf_linux_x64-6.0.8.jar" "$cache_path/SpedEcf.jar"
  ...
  user_install "SPED%20ECF%20v6.0.6"
